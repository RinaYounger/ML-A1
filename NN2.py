...
def plotLRLC(train_x, train_y):
    ...
    for x in range(len(lrs)):
        train_score, test_score = validation_curve(
            MLPClassifier(random_state=42, hidden_layer_sizes=(50,), alpha=1, learning_rate_init=lrs[x]), train_x, train_y,
            param_name="max_iter",
    ...
    plt.savefig('ReportNNSpam LR LC.png')
    plt.close()
    plt.figure()
...
def main(alpha=0, layers = None, lr = 0, mi = 0, train_x=None, test_x=None, train_y=None, test_y=None):
    ...
    #                                            cv=5, scoring="accuracy")
    #
    # plotValidationCurves(train_score, test_score, title="NN alpha Validation Curve", name="ReportNNSpam VC 1",
    ...
    # train_score, test_score = validation_curve(MLPClassifier(random_state=42, alpha=1), train_x, train_y,
    #                                            param_name="hidden_layer_sizes",
    #                                            param_range=parameter_range,
    #                                            cv=5, scoring="accuracy")
    # # parameter_range = ['(10, 10,)', '(10, 20)', '(10, 30)', '(10, 50)', '(10, 100)', '(10, 150)']
    # parameter_range = ['(1,)', '(10,)', '(20,)', '(30,)', '(50,)', '(100,)', '(150)']
    # plotValidationCurves(train_score, test_score, title="NN Validation Curve Layers alpha = 1", name="ReportNNSpam VC layer 1",
    ...
    #     MLPClassifier(hidden_layer_sizes=(50,), alpha=1, random_state=42, learning_rate_init = .001, max_iter=75), train_x, train_y,
    #     cv=5,
    #     scoring='accuracy',
    #     n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title="NN Learning Curve - Tuned",
    #                    name="ReportNNSpam LC 2 final")
