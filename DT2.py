...
def findBest(train_x, train_y, test_y, test_x):
    ...
    param_dict = {"criterion": ['gini', 'entropy'],
                  ...
                  "ccp_alpha" : [.0005, .001, .002, .003, .004, .005]}
...
def main(max_depth = 0, alpha = 0, train_x = None, test_x = None, train_y = None, test_y = None):
    ...
    #                                                         n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title = "DT Learning Curve Default Params", name = "ReportDTSpam LC 1" )

    # parameter_range = np.arange(1, 15)
    # train_score, test_score = validation_curve(DecisionTreeClassifier(), train_x, train_y,
    #                                            param_name="max_depth",
    #                                            param_range=parameter_range,
    #                                            cv=10, scoring="accuracy")
    #
    # plotValidationCurves(train_score, test_score, title = "DT Tree Depth Validation Curve Max Depth", name = "ReportDTSpam VC 1", param_name= "Max_Depth", parameter_range= parameter_range)


    # train_sizes, train_scores, test_scores = learning_curve(DecisionTreeClassifier(max_depth = 7), train_x, train_y, cv=10,
    #                                                         scoring='accuracy',
    #                                                         n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title="DT Learning Curve max_depth = 7", name="ReportDTSpam LC 2")

    # parameter_range = [.0005, .001, .002, .003, .004, .005]
    # train_score, test_score = validation_curve(DecisionTreeClassifier(max_depth=7), train_x, train_y,
    #                                            param_name="ccp_alpha",
    #                                            param_range=parameter_range,
    #                                            cv=10, scoring="accuracy")
    #
    # plotValidationCurves(train_score, test_score, title="DT ccp Alpha Validation Curve md = 7", name="ReportDTSpam VC 2",
    #                      param_name="alpha", parameter_range=parameter_range)

    # train_sizes, train_scores, test_scores = learning_curve(DecisionTreeClassifier(ccp_alpha = .004, max_depth=7) , train_x, train_y,
    #                                                         cv=10,
    #                                                         scoring='accuracy',
    #                                                         n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title="DT Learning Curve alpha = .004 and max depth = 7", name="RepoertDTSpam LC 3")

    # parameter_range = ['gini', 'entropy']
    # train_score, test_score = validation_curve(DecisionTreeClassifier(ccp_alpha = .004, max_depth=7), train_x, train_y,
    #                                            param_name="criterion",
    #                                            param_range=parameter_range,
    #                                            cv=5, scoring="accuracy")
    #
    # plotValidationCurves(train_score, test_score, title = "DT Criterion Validation Curve Criterion", name = "ReportDTSpam VC 4", param_name= "Criterion", parameter_range= parameter_range)
