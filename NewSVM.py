...
def plotKernelsGamma(train_x, train_y):
    ...
    for x in [2]:
        train_sizes, train_scores, test_scores = learning_curve(SVC(C=10, kernel='poly', degree=(x)), train_x, train_y,
    ...
    plt.savefig('ReportSVMBD LC 1.png')
    plt.close()
    plt.figure()
...
def plotTimes(train_x, train_y):
    ...
    plt.savefig('ReportSVMBD  Train Times.png')
    plt.close()
    plt.figure()
    ...
    plt.savefig('ReportSVMBD Test Times.png')
    plt.close()
    plt.figure()
...
def findBest(train_x, train_y):
    ...
    param_grid = {'C': [ 0.1, 1, 10, 100], 'gamma': [1, 0.1, .03, 0.01, 0.001]}
    estimator = SVC()
...
def plotCValues(train_x, train_y):
    ...
    plt.tight_layout()
    plt.legend(loc='best', prop={'size': 10})
    plt.savefig('ReportSVMBD VC 1.png')
    plt.close()
    plt.figure()
