...
def findBest(train_x, train_y):
    ...
    parameters_KNN = {'n_neighbors': range(1, 21, 1), 'p': [1, 2],
...
def main(n = 0, p = 0, train_x = None, test_x = None, train_y = None, test_y = None):
    ...
    #
    # plotValidationCurves(train_score, test_score, title = "KNN Num Neighbors Validation Curve", name = "ReportKNNSpam VC 1", param_name= "K", parameter_range = parameter_range)


    # train_sizes, train_scores, test_scores = learning_curve(KNeighborsClassifier(n_neighbors = 7), train_x, train_y, cv=5,
    #                                                         scoring='accuracy',
    #                                                         n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title="KNN Learning Curve Neighbors = 7", name="ReportKNNSpam LC 1")


    # parameter_range = [1, 2, np.inf]
    # train_score, test_score = validation_curve(KNeighborsClassifier(n_neighbors=7), train_x, train_y,
    #                                            param_name="p",
    #                                            param_range=parameter_range,
    #                                  cv=5, scoring="accuracy")
    ...
    # train_sizes, train_scores, test_scores = learning_curve(KNeighborsClassifier(n_neighbors=7, p = 1), train_x, train_y,
    #                                                         cv=5,
    #                                                         scoring='accuracy',
    #                                                         n_jobs=-1)
    #
    # plotLearningCurves(train_sizes, train_scores, test_scores, title="KNN Learning Curve K = 7 Manhattan", name="ReportKNNSpam LC 2")
    #
